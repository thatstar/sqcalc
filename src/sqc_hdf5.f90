...
contains
   ...
   !> Mean squared displacement MSD(t): the total plus one dataset per species
   !! under /msd/MSD_partial when the partials are written.
   subroutine hdf5_write_msd(path, axis, total, partial, labels, count, partials, nframes, &
      ...
      if (ierr == 0) call write_string_attr(file_id, 'quantity', 'msd', ierr, message)
      if (ierr == 0) call write_string_attr(file_id, 'axis', 'tau', ierr, message)
      if (ierr == 0) call write_string_attr(file_id, 'weight', 'unit', ierr, message)
      if (ierr == 0) call write_string_attr(file_id, 'norm', 'unit', ierr, message)
      if (ierr /= 0) then
      ...
      call h5gcreate_f(file_id, 'msd', group_id, hdferr)
      if (hdferr /= 0) then
         ierr = 1
         message = 'HDF5: cannot create the /msd group'
         call h5fclose_f(file_id, idum)
         call h5close_f(idum)
         return
      end if
      ...
      if (ierr == 0) then
         ...
         call write_dataset_f(group_id, 'MSD', H5T_NATIVE_DOUBLE, dims1, buf, ierr, message)
      end if
      if (ierr == 0) then
         cbuf = int(count, int64)
      ...
      if (partials .and. nspecies > 0) then
         ...
         if (ierr == 0) call write_string_dataset(group_id, 'pairs', label_buf, ierr, message)
         if (ierr == 0) call h5gcreate_f(group_id, 'MSD_partial', subgroup_id, hdferr)
         if (ierr == 0 .and. hdferr /= 0) then
            ierr = 1
            message = 'HDF5: cannot create the /msd/MSD_partial group'
         end if
         do p = 1, nspecies
            if (ierr /= 0) exit
